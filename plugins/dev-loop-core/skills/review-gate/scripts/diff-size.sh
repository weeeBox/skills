...
# reconciling two prose thresholds. BYTES=ERROR on a bad ref OR a failed `git diff` (e.g. no merge
# base), never a silent BYTES=0. Prints KEY=VALUE lines; exit 0 always (advisory to the skill).
#   diff-size.sh --selftest
set -uo pipefail
AGY_MAX=$((200*1024)); CODEX_WARN=$((50*1024))
disp() { # <bytes>
  local b="$1"
  printf 'BYTES=%s\n' "$b"
...
err() { echo "BYTES=ERROR"; echo "AGY=ERROR"; echo "CODEX=ERROR"; }
